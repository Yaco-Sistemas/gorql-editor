...
then
    ...
    hg clone https://hg.yaco.es/ceic-ogov-assistant /tmp/${PACKAGE}
fi
...
VERSION=`grep "Version: " ${PACKAGE}.spec | cut -d" " -f2 | cut -d"h" -f1`
TAG="${VERSION}hg${REV}"

echo "New version: ${TAG}"

sed -i "s/Version: [0-9].[0-9].[0-9]develhg[0-9]\+/Version: ${TAG}/g" ${PACKAGE}.spec
...
ln -s ${CURRENT_DIR}/${PACKAGE}.spec ${BUILD_ROOT}/SPECS/
